...
public class Diva_BossAttack_Projectile : Superclass_BossAttack_Projectile
{
    public override void ShootAtPlayer(GameObject target)
    {
        if (bossProjectileObject)
        {
            ...
            if (bossAttackProjectile != null)
            {
                ...
                bossAttackProjectile.InitializeDivaProjectile(projectileSpeed, projectileLifetime, direction, target);
            }
        }
    }
}